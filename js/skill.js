...
function Skill(name)
{
	...
	this.data = [
		...
		new StringValue('Template', 'template', 'NONE').setTooltip('Templates are presently not used (and are unlikely to ever be) - leave this as NONE.'),
                new ListValue('Texture Type', 'textureType', [ 'WEB', 'RESOURCE' ], 'RESOURCE').setTooltip('Set the texture link type. Note there is an issue with WEB textures where their icon appears as steve on the minimap, so it should be used temporarily only.'),
		new StringValue('Texture Link', 'textureUri', 'NONE').setTooltip('Link to a texture. This is either a CustomNPCs resource link or a web URL, depending on the textureType setting.'),
		new StringValue('Cape Texture Link', 'capeTexture', 'NONE').setTooltip('Cape texture link. This must be a CustomNPCs resource link, as web URL is not supported, though this is a rarely used setting.'),
		new StringValue('Overlay Texture Link', 'overlayTexture', 'NONE').setTooltip('Overlay texture link. This must be a CustomNPCs resource link, as web URL is not supported, though this is a rarely used setting.'),
		new ListValue('Faction', 'faction', factionList, 'Faction').setTooltip('NPC Faction Name. This changes the reputation rewards for killing this NPC and who this NPC is hostile with.'),
		new ListValue('Rotation Behavior', 'rotationBehavior', ['HeadRotation', 'NoRotation', 'RotateBody', 'Stalking' ], 'NoRotation').setTooltip('Rotation Behavior for this NPC. Most are self explanatory, except Stalking... Stalking will make this NPC turn and face the player as it approaches them and turn to keep things that way.'),
                new IntValue('Rotation Angle', 'rotation', 0).setTooltip('Angle for this NPC to be set to, if NoRotation or HeadRotation is set. 0 is EAST, 180 is WEST, 90 is NORTH and 270 is SOUTH.'),
		new ListValue('Visible', 'visible', ['YES', 'NO', 'PARTIAL' ], 'YES').setTooltip('Controls whethre or not this NPC is visible. PARTIAL is great for Hologram NPCs or Ghosts.'),
                new ListValue('Model', 'modelType', modelList, 'NONE').setTooltip('Class Name for this NPCs Model type. This can make the NPC appear to look like a dragon, for example.'),
                new ListValue('Gender', 'gender', [ 'MALE', 'FEMALE' ], 'MALE').setTooltip('Gender of this NPC. It enables or disables female appearance settings (like breasts).'),
		...
		new ListValue('Show Boss Bar', 'showBossBar', ['YES', 'NO', 'WHENATTACKING' ], 'NO').setTooltip('Controls whether or not this NPC has a boss bar enabled, and when it should be shown.'),
		new ListValue('Show Nameplate', 'showName', ['YES', 'NO', 'WHENATTACKING' ], 'NO').setTooltip('Controls whether or not this NPC has a nameplate enabled, and when it should be shown.'),
		...
		new ListValue('Melee Resistance', 'meleeResistance', ['NONE', 'LOWEST', 'LOW', 'MEDIUM','HIGH', 'HIGHEST', 'COMPLETE'], 'NONE').setTooltip('Whether or not the class requires a permission to be professed as. The permission would be "skillapi.class.{className}"'),
		new ListValue('Explosion Resistance', 'explosionResistance', ['NONE', 'LOWEST', 'LOW', 'MEDIUM','HIGH', 'HIGHEST', 'COMPLETE'], 'NONE').setTooltip('Whether or not the class requires a permission to be professed as. The permission would be "skillapi.class.{className}"'),
		new ListValue('Knockback Resistance', 'knockbackResistance', ['NONE', 'LOWEST', 'LOW', 'MEDIUM','HIGH', 'HIGHEST', 'COMPLETE'], 'NONE').setTooltip('Whether or not the class requires a permission to be professed as. The permission would be "skillapi.class.{className}"'),
		...
		new ListValue('Ranged Projectile Size', 'projectileSize', ['TINY' , 'SMALL', 'MEDIUM', 'LARGE', 'HUGE' ], 'TINY').setTooltip('What is the size of the NPCs projectile? A Bullet is TINY, an arrow would be MEDIUM and a staff blast is up to you!'),
                ...
                new ListValue('Projectile Trail Type', 'projectileTrail', [ 'NONE', 'Crit', 'Smoke', 'LargeSmoke', 'Enchant', 'Lightning', 'Portal', 'Magic', 'Redstone' ], 'NONE').setTooltip('Trail effect for the projectile.'),
		...
		new ListValue('Wandering Distance', 'wanderingDistance', wanderingList, '1').setTooltip('This is a predefined list of items and weapons NPCs are allowed to hold.'),
                ...
                new ListValue('Returns to Start', 'returnsToStart', ['YES', 'NO' ], 'YES').setTooltip('Whether or not the class requires a permission to be professed as. The permission would be "skillapi.class.{className}"'),
                new ListValue('Say Lines in Random Order', 'sayRandomLines', ['YES', 'NO' ], 'NO').setTooltip('Whether or not the class requires a permission to be professed as. The permission would be "skillapi.class.{className}"'),
